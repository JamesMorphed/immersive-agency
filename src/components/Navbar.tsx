  ...
  return (
    <header 
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        scrolled ? 'bg-black/80 backdrop-blur-md border-b border-cyberpunk-magenta/30' : 'bg-transparent'
      }`}
    >
      ...
      {/* Mobile menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-black/95 backdrop-blur-md border-y border-cyberpunk-cyan/30">
...
const NavLink = ({ href, children }: { href: string, children: React.ReactNode }) => {
  return (
    <a 
      href={href} 
      className="text-sm font-medium text-gray-300 hover:text-cyberpunk-magenta transition-colors relative group"
    >
      {children}
...
const MobileNavLink = ({ href, children, onClick }: { href: string, children: React.ReactNode, onClick: () => void }) => {
  return (
    <a 
      href={href} 
      className="block text-base font-medium text-gray-300 hover:text-cyberpunk-magenta transition-colors"
      onClick={onClick}
    >
      {children}
    </a>
